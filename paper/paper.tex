...
The final and somewhat unintuitive managerial issue with DNSSEC is what needs to be done once a registrant wishes to turn it off.

In such a case, it is absolutely insufficient to only turn off the service on the devices serving the aforementioned  zone: in doing so, resolvers won't be able to distinguish between spoofed DNS replies that contain non-DNSSEC data and legitimate replies that simply stopped using DNSSEC.

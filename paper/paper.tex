...
In the DNS world, the \emph{friend} we trust is the so-called root authority -- the root domain -- of whom we know the public key in advance.
...
update its DS record regularly.
...
verifies whether they are properly signed and then applies a heuristic to
...
These two entities can eventually be linked by the third one, the registrar: this is where registrants will obtain the actual registration service, namely, the linking between a domain name and an IP address. Figure~\ref{gif:3rs} illustrates the relations among the three R's.
...
Enabling DNSSEC for a zone not being a \emph{do once and forget} operation, but instead requires additional coordination among the three R's, let us see what the managerial consequences arise, depending on if we cross or stay within certain administrative boundaries.

...
DNSSEC trust chains are similar to the ones used in SSL, where an entity marks its trust in another by \emph{signing} its certificate\footnote{Which is nothing more than a public-private key pair with additional metadata.}

Hence, in the real world, the \emph{friend} we trust is the so-called root authority -- the root domain in DNSSEC -- of whom we know the public key in advance.

By signing the keys of the TLD's, the root will put its trust into their keys, and any keys the TLD's have signed, resulting in a trust hierarchy illustrated in \ref{fig:trust-chain}. This way, a resolver querying EPFL's DNS server will be able to verify the received query answer against the known root-authority in the following way:

...
In this paper we will first remind of the prinicpal necessary
operations for key management in DNSSEC. Then we will give the reader
an overview of the current status of DNSSEC deployment in the Internet
to put the in-detail discussion about key management into the right
context.
...
reader about specific details particularily relevant to this paper.
...
Whenever an entity wishes to sign its zone, it should ideally turn to its already signed parent zone, in order to create a continuous trust chain, as said parent will then serve a DS record containing the child zone's key signature.
